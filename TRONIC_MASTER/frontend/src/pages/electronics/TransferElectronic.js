...
import './TransferElectronic.css';

const TransferElectronic = () => {
  ...
  const fetchCurrentOwner = async () => {
    try {
      ...
      if (product.branch) {
        setCurrentOwner({
          type: 'branch',
          name: product.branch.name || 'Not Assigned',
  ...
  const autoAssignProductToAgent = async (agentId) => {
    try {
      const token = localStorage.getItem('token');
      console.log(`📱 Auto-assigning product ${product._id} to agent ${agentId}`);
  ...
  if (!product || !unit) {
    return (
      <MainLayout title="Transfer Electronic" breadcrumbs={['Home', 'Products', 'Electronics', 'Transfer']}>
  ...
  const getCurrentOwnerDisplay = () => {
    ...
    if (currentOwner.type === 'user') return `👤 ${currentOwner.name}`;
    if (currentOwner.type === 'branch') return `🏢 ${currentOwner.name}`;
    return '—';
  };

  const getCurrentBranchDisplay = () => {
    return branch?.name || product?.branch?.name || 'Not Assigned';
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('📤 Transferring Serial:', transferData);

      const response = await fetch(`${API_URL}/products/electronics/${product._id}/transfer-unit`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      if (data.success) {
        if (formData.transferType === 'user' && selectedUser) {
          ...
          if (assigned) console.log('✅ Product auto-assigned to agent');
        }
        
        alert(`✅ Serial ${unit.identifier} transferred successfully!`);
    ...
    <MainLayout title="Transfer Electronic" breadcrumbs={['Home', 'Products', 'Electronics', 'Transfer']}>
      <div className="transfer-page">
        <div className="transfer-header">
          ...
          <h2>🔄 Transfer Serial</h2>
          <p>Transfer this serial number to another user or branch</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="transfer-form">
                ...
                <>
                  <div className="search-container">
                    <input
                      ...
                      autoComplete="off"
                    />
                    {searchLoading && <div className="search-spinner"><div className="spinner-small"></div></div>}
                    {searchTerm && (
                      <button
                        type="button"
                        ...
                        filteredUsers.map(user => (
                          ...
                          >
                            ...
                            <div className="user-item-info">
                              ...
                              {user.phone && <span className="user-item-phone">📞 {user.phone}</span>}
                            ...
                            {selectedUser?._id === user._id && <span className="user-item-check">✓</span>}
                          </div>
                        ))
                      ) : (
          ...
          {formData.transferType === 'branch' && (
            <div className="form-group">
              ...
              {branches.length === 0 && <p className="no-data">No other branches available</p>}
            </div>
          )}
          ...
          <div className="form-group">
            ...
            <textarea
              ...
              placeholder="Please explain why this serial is being transferred..."
              rows="3"
              required
            />
          </div>
          ...
          <div className="transfer-summary">
            ...
            <div className="summary-row">
              ...
              <span className="serial-value">{unit.identifier}</span>
            </div>
            <div className="summary-row">
            ...
            {formData.transferType === 'user' && selectedUser && (
              <div className="summary-row auto-assign-info">
                ...
                <span className="auto-assign-text">Product will be auto-assigned to agent</span>
              </div>
            )}
          </div>

          <div className="warning-box">
            ⚠️ This action will transfer ownership of this serial to the selected recipient
          </div>
...
export default TransferElectronic;